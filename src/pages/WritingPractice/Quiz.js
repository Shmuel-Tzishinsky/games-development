...
export default function Quiz(props){
  ...
  useEffect(() => {
    setQuestion(
      quizData.filter(e => e.level === selectLevel)[0]
    )
  ...
  function setNextLevel_func(suc, mis){
    if(mis >= question.mistakes){
      // כאשר המשתמש לא עבר את השלב (מציג את דף 'נכשל')
      setFinishFails(true)
    }else{
        ...
        <div className="practice">
          { !userName ? (
                ...
                />
                  ...
                  <FinishFails
                  setFinishFails={setFinishFails}
                  level={selectLevel}
                  />
                  ) : finishSucceeded ? (
                    <FinishSucceeded
                    setFinishSucceeded={setFinishSucceeded}
                    level={selectLevel}
                  />
              ) : succeededEverything ? (
                  <SucceededEverything
                    setSucceededEverything={setSucceededEverything}
                    level={selectLevel}
                  />
              ) : (
                <Trivia
